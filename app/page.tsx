    ...
    <div style={{ fontFamily: "'Montserrat', 'Inter', -apple-system, sans-serif", minHeight: '100vh', background: '#0A0D14', color: '#fff', overflowX: 'hidden' }}>
      ...
      <section className="sec-pad" style={{
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 17, color: 'rgba(255,255,255,0.65)', lineHeight: 1.8, maxWidth: 600, margin: '0 auto 48px' }}>
          ...
          <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginBottom: 24, letterSpacing: 0.5, textTransform: 'uppercase', fontWeight: 600 }}>Your Journey</p>
          <div className="timeline-row" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center' }}>
            {([
              { yr: 'Year 1', label: 'Onboarding &\nProduction begins', gold: false },
              { yr: 'Year 2–5', label: 'Validation period\n& milestone checks', gold: false },
              { yr: 'Year 6', label: 'Full validation\ncomplete', gold: false },
              { yr: 'Year 7', label: 'Bonus extension\n(outperformers)', gold: true },
            ] as Array<{yr: string; label: string; gold: boolean}>).map(({ yr, label, gold }, i) => (
              <div key={yr} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                <div style={{ textAlign: 'center', flex: 1 }}>
                  <div style={{ width: 44, height: 44, borderRadius: '50%', background: gold ? GOLD : 'rgba(255,255,255,0.08)', border: `2px solid ${gold ? GOLD : 'rgba(255,255,255,0.2)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 10px', fontSize: 11, fontWeight: 800, color: gold ? '#000' : '#fff' }}>
                    {i + 1}
                  </div>
                ...
                {i < 3 && <div className="timeline-connector" style={{ flex: 0.3, height: 1, background: 'rgba(255,255,255,0.15)', marginBottom: 30 }} />}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="sec-pad" style={{
        ...
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <h2 style={{ textAlign: 'center', fontSize: 22, fontWeight: 700, color: '#fff', marginBottom: 40 }}>
            Who is MAP 8 for?
          </h2>
          <div className="cards-row" style={{ display: 'flex', gap: 20, marginBottom: 48 }}>
            {CARDS.map(({ img, badge, title, desc }) => (
              <div key={title} className="audience-card" onClick={scrollToForm}>
                <div style={{ position: 'relative' }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={img} alt={title} className="card-img" />
                  <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, transparent 50%, rgba(0,0,0,0.12))' }} />
                  <span style={{ position: 'absolute', top: 12, left: 14, fontSize: 10, fontWeight: 700, background: `${GOLD}EE`, color: '#000', padding: '3px 10px', borderRadius: 50, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{badge}</span>
                </div>
          ...
          <div style={{ textAlign: 'center' }}>
            <button className="btn-pill" onClick={scrollToForm}>Find Out If You Qualify &rsaquo;</button>
          </div>
        </div>
      </section>
      ...
      <section className="sec-pad" style={{
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 'clamp(22px, 3vw, 34px)', fontWeight: 800, color: '#fff', marginBottom: 56, letterSpacing: '-0.3px' }}>How it works</h2>
          <div className="steps-row" style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
            {[
              ...
              { n: '02', title: 'Director reviews your profile', desc: 'Our director personally reviews every application and prepares a tailored proposal based on your history.' },
              ...
              <div key={n} style={{ flex: 1, textAlign: 'center', padding: '0 12px', position: 'relative' }}>
                {i < 3 && <div className="timeline-connector" style={{ position: 'absolute', right: -8, top: 20, fontSize: 22, color: 'rgba(212,175,55,0.35)', fontWeight: 300 }}>›</div>}
                <div style={{ width: 44, height: 44, borderRadius: '50%', border: `1.5px solid ${GOLD}60`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: 13, fontWeight: 800, color: GOLD }}>{n}</div>
          ...
          <div style={{ marginTop: 52 }}>
            <button className="btn-pill" onClick={scrollToForm}>Start the Conversation &rsaquo;</button>
          </div>
        </div>
      </section>
          ...
          <div style={{ border: `1px solid rgba(212,175,55,0.3)`, borderRadius: 20, padding: '40px 38px', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(12px)' }}>
            {submitted ? (
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom: 14 }}>
                  <select value={form.income} onChange={e => setForm(f => ({ ...f, income: e.target.value }))} style={{ ...inputStyle, cursor: 'pointer', color: form.income ? '#fff' : 'rgba(255,255,255,0.35)' }} required>
                  ...
                  <div style={{ display: 'flex', gap: 28 }}>
                    {['Yes', 'No'].map(v => (
                      <label key={v} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14, color: 'rgba(255,255,255,0.8)', fontWeight: 500 }}>
                        <input type="radio" name="mdrt" value={v.toLowerCase()} checked={form.mdrt === v.toLowerCase()} onChange={() => setForm(f => ({ ...f, mdrt: v.toLowerCase() }))} style={{ accentColor: GOLD, width: 16, height: 16 }} />
                        {v}
                      </label>
                    ))}
                  </div>
                ...
                <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer', marginBottom: 20 }}>
                  <input type="checkbox" checked={form.consent} onChange={e => setForm(f => ({ ...f, consent: e.target.checked }))} style={{ marginTop: 3, accentColor: GOLD, width: 14, height: 14, flexShrink: 0 }} />
                ...
                {error && <div style={{ background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.3)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#FCA5A5', marginBottom: 16 }}>{error}</div>}
